...
@contextmanager
def raw_pretty(play_info):
    from rich.pretty import pprint
    pprint(play_info)


@contextmanager
def info_link(play_info):
...
async def get_play_url(room_id: int,
    ...
    from .clients import BilibiliCookieClient
    ...
    with out_mgr(play_info) as each:
        if callable(each):
