...
async def test_observer():
    ...
    with (
        patch('ubw.app.observer.Observer', spec=Observer) as patch_observer_handler_class,
        patch('ubw.app.observer.WSWebCookieLiveClient', spec=WSWebCookieLiveClient) as patch_live_client_class,
        patch('rich.print') as patch_rich_print
    ):
        async def astart(c):
            checkpoint2.reach()
